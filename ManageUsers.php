			...
			<form enctype="multipart/form-data">
				...
				<fieldset>
					...
					<label>
						...
						<br><br>
						<label for="class_group"><b>Class Group:</b></label>
						<select class="dev_sel" name="class_group" id="class_group" style="color: #000;" required>
							<option value="">-- Select Class --</option>
							<?php
							$groups = $conn->query("SELECT cg.group_id, al.abbreviation AS level, 
							...
							while ($group = $groups->fetch_assoc()):
								$class_name = $group['level'] . 
											 ($group['specialty'] != 'TC' ? '-' . $group['specialty'] : '') . 
											 '-TD' . $group['group_number'] . 
											 '-TP' . $group['group_number'];
							?>
								<option value="<?= $group['group_id'] ?>"><?= $class_name ?></option>
							<?php endwhile; ?>
						</select>
						<br><br>
